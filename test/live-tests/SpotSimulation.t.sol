...
contract SimulationTest is Test {
    ...
    function setUp() public {
        ...
        distributor = Distributor(vm.envOr("GTE_DISTIBUTOR_TESTNET", address(0)));
        operator = OperatorPanel(vm.envOr("GTE_OPERATOR_TESTNET", address(0)));
    ...
    //     hops.push(
    //         bytes.concat(
    //             abi.encodePacked(GTERouter.uniV2SwapExactTokensForTokens.selector),
    ...
    function _upgradeCLOBManager() internal {
        ...
        address cmLogic = address(new CLOBManager(address(beacon), address(operator)));
    ...
    function _upgradeCLOB() internal {
        ...
        // address clobLogic = address(new CLOB(address(gteRouter), address(operator), 2_147_483_647, address(0), address(0)));
    ...
    function _upgradeLaunchpad() internal {
        address launchpadLogic = address(
            new Launchpad(
                address(uniV2Router), address(gteRouter), address(clobManager), address(operator), address(distributor)
            )
        );
